...
var Field;
(function(Field2) {
  Field2.numberTypes = ["integer", "unsigned", "float", "double"];
  Field2.stringTypes = ["char", "string", "text"];
  Field2.dateTypes = ["timestamp", "date", "time"];
  Field2.objectTypes = ["list", "json"];
})(Field || (Field = {}));
var Tables;
(function(Tables3) {
  Tables3.config = {};
  function extend(name, meta = {}) {
    const { unique = [], foreign, fields } = Tables3.config[name] || {};
    Tables3.config[name] = __spreadProps(__spreadValues({
      type: "incremental",
      primary: "id"
    }, meta), {
      unique: [...unique, ...meta.unique || []],
      foreign: __spreadValues(__spreadValues({}, foreign), meta.foreign),
      fields: __spreadValues(__spreadValues({}, fields), meta.fields)
    });
  }
  Tables3.extend = extend;
  function create(name) {
    const { fields } = Tables3.config[name];
    const result = {};
    for (const key in fields) {
      if (fields[key].initial !== void 0) {
