...
export namespace Field {
    const numberTypes: Type[];
    const stringTypes: Type[];
    const dateTypes: Type[];
    const objectTypes: Type[];
    type Type<T = any> = T extends number ? 'integer' | 'unsigned' | 'float' | 'double' : T extends string ? 'char' | 'string' | 'text' : T extends Date ? 'timestamp' | 'date' | 'time' : T extends any[] ? 'list' | 'json' : T extends object ? 'json' : never;
}
...
export namespace Query {
    ...
    type Field<T extends TableType> = string & keyof Tables[T];
    type Index<T extends TableType> = IndexKeys<Tables[T], IndexType>;
    ...
    type Shorthand<T = IndexType> = T | T[] | RegExp;
    type FieldQuery<T> = FieldExpr<T> | Shorthand<T>;
    type Expr<T = any> = LogicalExpr<T> & {
        [K in keyof T]?: FieldQuery<T[K]>;
    };
    function resolve<T extends TableType>(name: T, query: Query<T>): Expr<Tables[T]>;
    interface Options<T extends string> {
        limit?: number;
        offset?: number;
        fields?: T[];
    }
    type Modifier<T extends string = any> = T[] | Options<T>;
    function resolveModifier<T extends string>(modifier: Modifier<T>): Options<T>;
    interface Database {
...
export namespace Argv {
    ...
    export type OptionType<S extends string, T extends Type> = [T] extends [DomainType] ? Domain[T] : [T] extends [RegExp] ? string : T extends (source: string) => infer R ? R : ExtractFirst<Replace<S, '>', ']'>, any>;
...
export class Command<U extends User.Field = never, G extends Channel.Field = never, A extends any[] = any[], O extends {} = {}> extends Argv.CommandBase {
    ...
    option<K extends string, D extends string, T extends Argv.Type>(name: K, desc: D, config?: Argv.OptionConfig<T>): Command<U, G, A, Extend<O, K, Argv.OptionType<D, T>>>;
    match(session: Session): boolean;
